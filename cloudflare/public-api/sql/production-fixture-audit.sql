...
SELECT
  CASE CAST(candidate.value AS INTEGER)
    WHEN 0 THEN 'public_home_sections'
    WHEN 1 THEN 'documents'
    WHEN 2 THEN 'contents'
    WHEN 3 THEN 'contents'
    WHEN 4 THEN 'visitor_daily_stats'
    WHEN 5 THEN 'visitor_daily_stats'
  END AS source,
  CASE CAST(candidate.value AS INTEGER)
    WHEN 0 THEN 'sample-public-read-home-section-001'
    WHEN 1 THEN 'sample-public-read-document-001'
    WHEN 2 THEN 'sample-public-read-content-001'
    WHEN 3 THEN 'sample-public-read-program-001'
    WHEN 4 THEN '2026-06-13'
    WHEN 5 THEN '2026-01-01'
  END AS fixture_key
FROM json_each('[0,1,2,3,4,5]') AS candidate
