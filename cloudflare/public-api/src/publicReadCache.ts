...
export function isPublicReadCacheEligible(request: Request, env: Env) {
  // Client Cache-Control/Pragma headers are deliberately ignored here. These are
  // anonymous public reads, so allowing callers to force cache misses would let a
  // crawler or bot bypass the shared edge cache and amplify D1 reads on demand.
