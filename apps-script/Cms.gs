function setupCmsBackend() {
  ...
  ensureSheet(spreadsheet, SHEETS.documents, DOCUMENT_HEADERS);
...
function getPublicHomeSnapshot() {
  ...
  const legacyDocumentItems = publicContent
    .filter((item) => item.type === "page" && hasHomeContentKeyword(item, PUBLIC_HOME_DOCUMENT_KEYWORDS))
    .slice(0, PUBLIC_HOME_DOCUMENT_LIMIT);
  const documentItems = publicDocuments.length
    ? publicDocuments.slice(0, PUBLIC_HOME_DOCUMENT_LIMIT)
    : legacyDocumentItems;
