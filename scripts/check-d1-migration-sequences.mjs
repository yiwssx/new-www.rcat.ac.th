...
const legacy0007 = new Set(["0007_admin_user_profiles.sql", "0007_public_analytics_abuse_guard.sql"]);
const LEGACY_DUPLICATES = new Map([["0007", legacy0007]]);

export function validateD1MigrationNames(fileNames) {
  const errors = [];
  const sequences = new Map();
  const sqlFileNames = fileNames.filter((name) => name.endsWith(".sql")).sort();
  ...
  for (const [sequence, legacyNames] of LEGACY_DUPLICATES) {
    ...
    for (const legacyName of legacyNames) {
      if (!currentNames.has(legacyName)) {
        errors.push(
          `Historical migration ${legacyName} must remain present; ` + "do not rename applied D1 migrations."
        );
      }
    }
  }
...
function main() {
  ...
  console.log(
    "D1 migration sequence validation passed. Legacy sequence 0007 remains " + "frozen and no new duplicates exist."
  );
}
