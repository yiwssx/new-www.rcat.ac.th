...
const REPAIR_ACTOR = "facebook-reclassifier";
...
function flattenWranglerRows(payload) {
...
function isFacebookImportRow(row) {
  ...
  return (
    (id.startsWith("facebook-post-") && template === "facebook-embed") ||
    (slug.startsWith("facebook-") && (owner === "facebook-import" || createdBy === "facebook-import"))
  );
}
...
function buildReportRow(row, indexes) {
  ...
  const currentCategoryRaw = String(row.category ?? "");
  const currentCategory = currentCategoryRaw.trim();
  const currentTagsJsonRaw = String(row.tags_json ?? "[]");
  const currentTags = parseTagsJson(currentTagsJsonRaw);
  const base = {
    id: String(row.id ?? ""),
    slug: String(row.slug ?? ""),
    sourceId: match.post?.id ?? "",
...
WHERE id = ${sqlLiteral(row.id)}
  ...
  AND category = ${sqlLiteral(row.currentCategoryRaw)}
  AND tags_json = ${sqlLiteral(row.currentTagsJsonRaw)};`;
}
...
async function runAudit({ d1Path, facebookPath, outputDir, batchSize }) {
...
async function runVerification({ d1Path, reportPath, outputDir }) {
  ...
  console.log(
    JSON.stringify(
      { expectedRepairs: result.expectedRepairs, verified: result.verified, mismatches: mismatches.length },
      null,
      2
    )
  );

  if (mismatches.length) throw new Error(`Verification failed for ${mismatches.length} repaired row(s)`);
}

async function runCli() {
  const args = parseArgs(process.argv.slice(2));
  const d1Path = requireText(args["d1-json"], "--d1-json");
  const outputDir = args["output-dir"]?.trim() || DEFAULT_OUTPUT_DIR;
